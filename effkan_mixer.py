...
class KANMixer(nn.Module):
    def __init__(self,in_channels=3,img_size=32, patch_size=4, hidden_size=512, hidden_s=256, hidden_c=2048, num_layers=8, num_classes=10, drop_p=0., off_act=False, is_cls_token=False, base_activation = F.silu, use_pe = False, enable_standalone_scale_spline = False):
        super(KANMixer, self).__init__()
        num_patches = img_size // patch_size * img_size // patch_size
        # (b, c, h, w) -> (b, d, h//p, w//p) -> (b, h//p*w//p, d)
        self.is_cls_token = is_cls_token

        print(f"Polynomial Basis: False, Degree of Polynomial: None, Using Same Function: False, Using same weights: False, Positional Embeddings: {use_pe}")
       ...
       #     ]
        ...
        self.mixer_layers = nn.Sequential(
            *[
                MixerLayer(num_patches, hidden_size, hidden_s, hidden_c, drop_p, off_act, base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline) 
...
class MixerLayer(nn.Module):
    def __init__(self, num_patches, hidden_size, hidden_s, hidden_c, drop_p, off_act, base_activation = F.silu, enable_standalone_scale_spline = False):
        super(MixerLayer, self).__init__()
        self.kan1 = KAN1(num_patches, hidden_s, hidden_size, drop_p, off_act, base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline)
        self.kan2 = KAN2(hidden_size, hidden_c, drop_p, off_act, base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline)
    def forward(self, x):
        out = self.kan1(x)
        out = self.kan2(out)
        return out
...
class KAN1(nn.Module):
    def __init__(self, num_patches, hidden_s, hidden_size, drop_p, off_act,base_activation = F.silu, enable_standalone_scale_spline = False):
        ...
        self.fc1 = KANLinear(num_patches, hidden_s, base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline)
        self.do1 = nn.Dropout(p=drop_p)
        #self.fc2 = nn.Conv1d(hidden_s, num_patches, kernel_size=1)
        self.fc2 = KANLinear(hidden_s, num_patches, base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline)
...
class KAN2(nn.Module):
    def __init__(self, hidden_size, hidden_c, drop_p, off_act, base_activation = F.silu, enable_standalone_scale_spline = False):
        super(KAN2, self).__init__()
        self.ln = nn.LayerNorm(hidden_size)
        self.fc1 = KANLinear(hidden_size, hidden_c,base_activation = base_activation, enable_standalone_scale_spline = enable_standalone_scale_spline)
        self.do1 = nn.Dropout(p=drop_p)
