...
class KAT_Group_Torch(nn.Module):
    def __init__(self, num_groups=1, mode="gelu"):
        """
...
class KAT(nn.Module):
    ...
    def __init__(
        ...
        self.act1 = KAT_Group_Torch(mode="identity")
        self.drop1 = nn.Dropout(drop_probs[0])
        self.norm1 = norm_layer(in_features) if norm_layer is not None else nn.Identity()
        self.norm2 = norm_layer(hidden_features) if norm_layer is not None else nn.Identity()
        self.act2 = KAT_Group_Torch(mode=act_init)
