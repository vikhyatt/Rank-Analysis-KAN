...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., num_grids = 8, use_same_fn = False):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        #self.fc1 = nn.Conv2d(in_features, hidden_features, 1)
        self.fc1 = KANLinear(in_features, hidden_features, num_grids = num_grids, use_same_fn = use_same_fn)
        self.act = nn.GELU()
        #self.fc2 = nn.Conv2d(hidden_features, out_features, 1)
        self.fc2 = KANLinear(hidden_features, out_features, num_grids = num_grids, use_same_fn = use_same_fn)
...
class HireMLP(nn.Module):
    def __init__(self, dim, attn_drop=0., proj_drop=0., pixel=2,
                 step=1, step_pad_mode='c', pixel_pad_mode='c', num_grids = 8, use_same_fn = False):
        super().__init__()
        """
        ...
        self.mlp_h1 = KANLinear(dim*pixel, dim//2, num_grids = num_grids, use_same_fn = use_same_fn)
        self.mlp_h1_norm = nn.BatchNorm2d(dim//2)
        #self.mlp_h2 = nn.Conv2d(dim//2, dim*pixel, 1, bias=True)
        self.mlp_h2 = KANLinear(dim//2, dim*pixel, num_grids = num_grids, use_same_fn = use_same_fn)
        #self.mlp_w1 = nn.Conv2d(dim*pixel, dim//2, 1, bias=False)
        self.mlp_w1 = KANLinear(dim*pixel, dim//2, num_grids = num_grids, use_same_fn = use_same_fn)
        self.mlp_w1_norm = nn.BatchNorm2d(dim//2)
        #self.mlp_w2 = nn.Conv2d(dim//2, dim*pixel, 1, bias=True)
        self.mlp_w2 = KANLinear(dim//2, dim*pixel, num_grids = num_grids, use_same_fn = use_same_fn)
        #self.mlp_c = nn.Conv2d(dim, dim, 1, bias=True)
        self.mlp_c = KANLinear(dim, dim, num_grids = num_grids, use_same_fn = use_same_fn)
        self.act = nn.ReLU()
        self.reweight = Mlp(dim, dim // 4, dim * 3, num_grids = num_grids, use_same_fn = use_same_fn)
        #self.proj = nn.Conv2d(dim, dim, 1)
        self.proj = KANLinear(dim, dim, num_grids = num_grids, use_same_fn = use_same_fn)
    ...
    def forward(self, x):
        ...
        h = self.act(h).permute(0,3,2,1)
        h = self.mlp_h2(h).permute(0,3,2,1)

        w = w.permute(0,3,2,1)
        w = self.mlp_w1(w).permute(0,3,2,1)
        w = self.mlp_w1_norm(w)
        w = self.act(w).permute(0,3,2,1)
        w = self.mlp_w2(w).permute(0,3,2,1)
        
        h = h.reshape(B, C, self.pixel, (H + pad_h) // self.pixel, W)
        h = h.permute(0, 1, 3, 2, 4)
        h = h.reshape(B, C, H + pad_h, W)
        w = w.reshape(B, C, self.pixel, H, (W + pad_w) // self.pixel)
        w = w.permute(0, 1, 3, 4, 2)
        w = w.reshape(B, C, H, W + pad_w)
        ...
        x_clone = x.permute(0,3,2,1)
        c = self.mlp_c(x_clone).permute(0,3,2,1)

        a = (h + w + c).flatten(2).mean(2).unsqueeze(2)
        a = a.unsqueeze(2)
        a = self.reweight(a).reshape(B, C, 3)
        a = a.permute(2, 0, 1).softmax(dim=0)
        a = a.unsqueeze(3)
        a = a.unsqueeze(3)
...
class HireBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=4., drop=0., attn_drop=0., drop_path=0.,
                 pixel=2, step=1, step_pad_mode='c', pixel_pad_mode='c', num_grids = 8, use_same_fn = False):
        super().__init__()
        self.norm1 = nn.BatchNorm2d(dim)
        self.attn = HireMLP(
            dim, attn_drop=attn_drop, pixel=pixel, step=step,
            step_pad_mode=step_pad_mode, pixel_pad_mode=pixel_pad_mode, num_grids = num_grids, use_same_fn = use_same_fn )

        self.norm2 = nn.BatchNorm2d(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, drop=drop, num_grids = num_grids, use_same_fn = use_same_fn)
...
def basic_blocks(dim, index, layers, mlp_ratio=4., attn_drop=0., drop_path_rate=0.,
                 ...
                 step_pad_mode='c', pixel_pad_mode='c', num_grids = 8 , use_same_fn = False,**kwargs):
    blocks = []
...
class HireKANNet(nn.Module):
    def __init__(self, layers, img_size=224, patch_size=4, in_chans=3, num_classes=100,
        ...
        step_pad_mode='c', pixel_pad_mode='c', num_grids = 8, use_same_fn = False):
        super().__init__()
        ...
        grid_sizes = [num_grids for _ in range(len(layers)//2)] + [num_grids//2 for _ in range(len(layers) - len(layers)//2)]
        ...
        for i in range(len(layers)):
            stage = basic_blocks(
                ...
                num_grids = grid_sizes[i], use_same_fn = use_same_fn)
        ...
        self.head = KANLinear(embed_dims[-1], num_classes, num_grids = grid_sizes[-1], use_same_fn = use_same_fn) if num_classes > 0 else nn.Identity()
