...
from fasterkan import FasterKANLayer as KANLinear
...
class FasterKANMixer(nn.Module):
    def __init__(self,in_channels=3,img_size=32, patch_size=4, hidden_size=512, hidden_s=256, hidden_c=2048, num_layers=8, num_classes=10, drop_p=0., off_act=False, is_cls_token=False, use_poly = False, degree_poly = 3, use_base_update = True, base_activation = F.silu, use_same_fn = False, use_same_weight = False, use_pe = False, use_cpd = False, use_softmax_prod = False):
        ...
        self.patch_emb = nn.Sequential(
            ...
            KANLinear((patch_size ** 2) * in_channels, hidden_size, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
        )
       ...
       #     ]
        ...
        self.clf = KANLinear(hidden_size, num_classes, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
...
class MixerLayer(nn.Module):
    def __init__(self, num_patches, hidden_size, hidden_s, hidden_c, drop_p, off_act, use_poly = False, degree_poly = 3, use_base_update = True, base_activation = F.silu, use_same_fn = False, use_same_weight = False, use_cpd = False, use_softmax_prod = False):
        super(MixerLayer, self).__init__()
        self.kan1 = KAN1(num_patches, hidden_s, hidden_size, drop_p, off_act, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
        self.kan2 = KAN2(hidden_size, hidden_c, drop_p, off_act, use_poly = use_poly, degree_poly = degree_poly, use_base_update= use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
    def forward(self, x):
        out = self.kan1(x)
        out = self.kan2(out)
        return out
...
class KAN1(nn.Module):
    def __init__(self, num_patches, hidden_s, hidden_size, drop_p, off_act, use_poly = False, degree_poly = 3,
                  use_base_update = True, base_activation = F.silu, use_same_fn = False, use_same_weight = False, use_cpd = False, use_softmax_prod = False):
        super(KAN1, self).__init__()
        self.ln = nn.LayerNorm(hidden_size)
        
        #self.fc1 = nn.Conv1d(num_patches, hidden_s, kernel_size=1)
        self.fc1 = KANLinear(num_patches, hidden_s, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
        self.do1 = nn.Dropout(p=drop_p)
        #self.fc2 = nn.Conv1d(hidden_s, num_patches, kernel_size=1)
        self.fc2 = KANLinear(hidden_s, num_patches, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
    ...
    def forward(self, x):
        ...
        return out+initial_x

class KAN2(nn.Module):
    def __init__(self, hidden_size, hidden_c, drop_p, off_act, use_poly = False, degree_poly = 3, use_base_update = True, base_activation = F.silu, use_same_fn = False, use_same_weight = False, use_cpd = False, use_softmax_prod = False):
        super(KAN2, self).__init__()
        self.ln = nn.LayerNorm(hidden_size)
        self.fc1 = KANLinear(hidden_size, hidden_c,use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
        self.do1 = nn.Dropout(p=drop_p)
        self.fc2 = KANLinear(hidden_c, hidden_size, use_poly = use_poly, degree_poly = degree_poly, use_base_update = use_base_update, base_activation = base_activation, use_same_fn = use_same_fn, use_same_weight = use_same_weight, use_cpd = use_cpd, use_softmax_prod = use_softmax_prod)
        self.do2 = nn.Dropout(p=drop_p)
        self.act = F.gelu if not off_act else lambda x:x
    def forward(self, x):
